...
│   └── ch03.h
├── ch04_trees/                         第 4 章 树（4.1 ~ 4.46 全部完成）
│   ├── ANSWERS.md                      含 46 题对照表 + 4.1~4.46 推导
│   ├── tree_core.c                     通用二叉树：遍历、统计、表达式树、画树
│   ├── bst.c                           4.10~4.14 BST、三种删除、FindKth、区间打印
│   ├── avl.c                           4.15~4.22 AVL（递归/非递归插入、删除、位宽）
│   ├── splay.c                         4.23~4.27 伸展树（旋转统计、顺序访问实验）
│   ├── btree.c                         4.36~4.38 B 树 / 2-3 树（插入分裂、删除借位）
│   ├── threaded.c                      4.45 右线索二叉树（中序不用栈/递归）
│   ├── kdtree.c                        4.46 2-d 树（插入、范围查找、最近邻）
│   └── ch04.h
...
| `tests/test_ch04.c` | 107797 | ✅ |
| **合计** | **593449** | ✅ 全部通过 |

另外四个测试都在 `-fsanitize=address,undefined` 下通过（无越界、无未定义行为）——
“游标邻接表 head 初值”“懒惰删除模型数组越界”“B 树奇数阶删除超容”
“2-d 树范围查找边界”等 bug 就是靠它抓出来的。
...
| 4 | 树（4.1 ~ 4.46） | ✅ 完成（46/46：BST/AVL/伸展树/B 树/线索树/2-d 树 + 证明题） |
| 5 | 散列 | ⏳ |
| 6 | 优先队列（堆） | ⏳ |
| 7 | 排序 | ⏳ |
| 8 | 不相交集 | ⏳ |
